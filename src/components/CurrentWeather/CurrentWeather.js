...
import { capitalize, getLocalDay, getWeatherImage, getWindDirection, importImages, uvIndexScale } from "../../utils";
...
const CurrentWeather = ({ weather, mainClass }) => {
    ...
    return (
        <div className="p-2 mx-2 my-3 current-weather">
            <div className="container cw-display">
                <div className="city">{weather.location.label && weather.location.label}</div>
                <div className="date mb-2">{(current.datetimeEpoch && weather.weatherData.tzoffset) && getLocalDay(weather.weatherData.tzoffset, current.datetimeEpoch)}</div>
